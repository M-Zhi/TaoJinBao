...
- [x] 节假日感知：v15 已修复。内置 `assets/cn_holidays.json`（2025-2027 法定节假日 + 调休补班），`HolidayCalendar` 单例在 `Application.onCreate` 初始化，`TradingHoursUtils` 综合判断后正确显示"休市中"或"交易中"；调休补班周六也能正常记录开盘价
